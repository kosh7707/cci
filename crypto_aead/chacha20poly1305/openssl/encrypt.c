/*
 * ChaCha20-Poly1305 — OpenSSL EVP implementation
 * Based on backup/OpenSSL/chacha20poly1305 (proven SUPERCOP implementation)
 *
...
)
{
  ...
  if (!EVP_EncryptInit_ex(x,EVP_chacha20_poly1305(),0,0,0)) goto error;
  if (!EVP_CIPHER_CTX_ctrl(x,EVP_CTRL_AEAD_SET_IVLEN,12,0)) goto error;
  ...
  if (!EVP_CIPHER_CTX_ctrl(x,EVP_CTRL_AEAD_GET_TAG,16,c + mlen)) goto error;
...
)
{
  ...
  x = EVP_CIPHER_CTX_new(); if (!x) goto error;
  if (!EVP_DecryptInit_ex(x,EVP_chacha20_poly1305(),0,0,0)) goto error;
  if (!EVP_CIPHER_CTX_ctrl(x,EVP_CTRL_AEAD_SET_IVLEN,12,0)) goto error;
  if (!EVP_CIPHER_CTX_ctrl(x,EVP_CTRL_AEAD_SET_TAG,16,(unsigned char *) c + clen)) goto error;
